...
function renderSegmentContentSections(segment) {
  ...
  imagesSection.className = 'images prologue-images';
...
function renderPrologueContentSections(prologue) {
  const fragment = document.createDocumentFragment();

  const textSection = document.createElement('section');
  textSection.className = 'text-block prologue-text';
  (prologue.adventureText || []).forEach((paragraphText) => {
    const p = document.createElement('p');
    p.innerHTML = paragraphText;
    textSection.appendChild(p);
  });

  const imagesSection = createSectionWithHeading('images prologue-images', 'Images');
  if (!prologue.images || !prologue.images.length) {
    const noImages = document.createElement('p');
    noImages.textContent = 'No images recorded for this segment.';
    imagesSection.appendChild(noImages);
  } else {
    prologue.images.forEach((image) => {
      const figure = document.createElement('figure');
      figure.className = 'image-entry';
  ...
  fragment.append(textSection, imagesSection);
  return fragment;
}
...
function renderPrologue(prologue) {
  ...
  section.setAttribute('data-prologue-title', prologue.label || 'Prologue');
  ...
  const prologueMeta = document.createElement('div');
  prologueMeta.className = 'prologue-meta-line';
  const kicker = document.createElement('div');
  kicker.className = 'segment-kicker';
  [
//    prologue.label || 'Prologue',
    prologue.location || 'Location TBD'
  ].forEach((value) => {
    const span = document.createElement('span');
    span.textContent = value;
    kicker.appendChild(span);
  });
  prologueMeta.appendChild(kicker);

  const cardBody = document.createElement('div');
  cardBody.className = 'segment-body prologue-card-body';
  cardBody.appendChild(renderPrologueContentSections(prologue));

  content.append(prologueMeta, cardBody);
