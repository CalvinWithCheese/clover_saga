...
function buildToc() {
  ...
  document.querySelectorAll('.prologue').forEach((prologue) => {
    ...
    prologueLink.className = 'toc-link toc-link-prologue';
    prologueLink.href = `#${prologueId}`;
    prologueLink.textContent = prologueTitle;

    prologueHeader.appendChild(prologueLink);
    prologueLi.appendChild(prologueHeader);
    toc.appendChild(prologueLi);
  });

  document.querySelectorAll('.era:not(.prologue)').forEach((era) => {
    const eraId = era.id;
...
function renderSegmentContentSections(segment) {
  ...
  fragment.append(textSection, imagesSection, commentarySection, summarySection, stateSection);
  return fragment;
}
...
function renderSegment(segment, eraLabel, eraSlug, segmentIndex) {
  ...
  body.hidden = true;
  body.appendChild(renderSegmentContentSections(segment));
...
function renderPrologue(prologue) {
  ...
  const prologueCard = document.createElement('article');
  prologueCard.className = 'prologue-card';
  prologueCard.id = segment.slug;
  prologueCard.setAttribute('data-segment-number', String(segment.number || '000'));
  prologueCard.setAttribute('data-segment-title', segment.title || segment.slug);

  const cardHeader = document.createElement('div');
  cardHeader.className = 'prologue-card-header';

  const kicker = document.createElement('div');
  kicker.className = 'segment-kicker';
  [
    `Segment ${String(segment.number || 0).padStart(3, '0')}`,
  ...
  const title = document.createElement('h4');
  title.textContent = segment.title || 'Untitled Prologue';
  const tagline = document.createElement('p');
  tagline.textContent = segment.tagline || '[tagline]';
  cardHeader.append(kicker, title, tagline);

  const cardBody = document.createElement('div');
  cardBody.className = 'segment-body prologue-card-body';
  cardBody.appendChild(renderSegmentContentSections(segment));

  prologueCard.append(cardHeader, cardBody);
  content.appendChild(prologueCard);
