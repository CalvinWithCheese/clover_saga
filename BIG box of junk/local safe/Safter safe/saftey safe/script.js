...
function buildToc() {
  ...
  document.querySelectorAll('.prologue').forEach((prologue) => {
    ...
    prologueLink.addEventListener('click', () => {
...
function expandCollapseAll(expand) {
  document.querySelectorAll('.era-header, .segment-header[data-target], .collapsible-trigger').forEach((button) => {
...
function renderPrologue(prologue) {
  ...
  const prologueCard = renderSegment(segment, prologue.label || 'Prologue', prologueSlug, 0);
  const prologueSegmentHeader = prologueCard.querySelector('.segment-header');
  const prologueSegmentBody = prologueCard.querySelector('.segment-body');

  if (prologueSegmentHeader && prologueSegmentBody) {
    const staticHeader = document.createElement('div');
    staticHeader.className = 'segment-header';
    while (prologueSegmentHeader.firstChild) {
      staticHeader.appendChild(prologueSegmentHeader.firstChild);
    }
    prologueSegmentHeader.replaceWith(staticHeader);
    prologueSegmentBody.hidden = false;
  }
