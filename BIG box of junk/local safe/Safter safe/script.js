...
function buildToc() {
  ...
  document.querySelectorAll('.prologue').forEach((prologue) => {
    const prologueId = prologue.id;
    const prologueTitle = prologue.getAttribute('data-prologue-title') || 'Prologue';
    const tocPrologueId = `toc-${prologueId}-segments`;
    const prologueSegment = prologue.querySelector('.segment');

    const prologueLi = document.createElement('li');
    const prologueHeader = document.createElement('div');
    prologueHeader.className = 'toc-era-header';

    const prologueToggle = document.createElement('button');
    prologueToggle.type = 'button';
    prologueToggle.className = 'collapsible-trigger';
    prologueToggle.textContent = '▸';
    prologueToggle.setAttribute('aria-label', `Toggle ${prologueTitle} in table of contents`);
    prologueToggle.setAttribute('aria-expanded', 'false');
    prologueToggle.setAttribute('data-target', tocPrologueId);

    const prologueLink = document.createElement('a');
    prologueLink.className = 'toc-link';
    prologueLink.href = `#${prologueId}`;
    prologueLink.textContent = prologueTitle;

    prologueHeader.append(prologueToggle, prologueLink);

    const prologueSegmentsList = document.createElement('ul');
    prologueSegmentsList.id = tocPrologueId;
    prologueSegmentsList.className = 'toc-segments';
    prologueSegmentsList.hidden = true;

    if (prologueSegment) {
      const number = prologueSegment.getAttribute('data-segment-number') || '???';
      const title = prologueSegment.getAttribute('data-segment-title') || prologueSegment.id;
      const item = document.createElement('li');
      const link = document.createElement('a');
      link.className = 'toc-link';
      link.href = `#${prologueSegment.id}`;
      link.textContent = `Segment ${number} · ${title}`;
      link.addEventListener('click', () => {
        const prologueHeaderButton = prologue.querySelector('.era-header');
        const segmentHeaderButton = prologueSegment.querySelector('.segment-header');
        if (prologueHeaderButton) setExpanded(prologueHeaderButton, true);
        if (segmentHeaderButton) setExpanded(segmentHeaderButton, true);
      });
      item.appendChild(link);
      prologueSegmentsList.appendChild(item);
    }

    prologueLi.append(prologueHeader, prologueSegmentsList);
...
function renderPrologue(prologue) {
  const prologueSlug = prologue.slug || 'prologue';
  const segment = {
    ...prologue,
    slug: prologue.slug || 'prologue-segment-000'
  };

  const section = document.createElement('section');
  section.className = 'era prologue';
  section.id = prologueSlug;
  section.setAttribute('data-prologue-title', `${prologue.label || 'Prologue'} · ${prologue.title || 'Untitled'}`);

  const headerButton = document.createElement('button');
  headerButton.className = 'era-header';
  headerButton.type = 'button';
  headerButton.setAttribute('data-target', `${prologueSlug}-content`);
  headerButton.setAttribute('aria-expanded', 'false');

  const strong = document.createElement('strong');
  strong.textContent = prologue.label || 'Prologue';
  const span = document.createElement('span');
  span.textContent = prologue.subtitle || '';
  headerButton.append(strong, span);

  const content = document.createElement('div');
  content.className = 'era-content';
  content.id = `${prologueSlug}-content`;
  content.hidden = true;
  content.appendChild(renderSegment(segment, prologue.label || 'Prologue', prologueSlug, 0));
